<template>
  <div v-for="item in promiseResult">
    <ul class="main-tree">
      <li class="tree-node" @click="onItemSelect(item.id)">&gt; {{ item.name }}</li>
    </ul>
  </div>
</template>
    ...
    currentMenuFilter() {
      this.dataView.then(value => {
        if (this.currentMenuFilter.trim() === '') {
          ...
          this.promiseResult = value.filter(x => x.name.trim().toLowerCase().includes(this.currentMenuFilter.trim().toLowerCase()))
        }
      })
    }
  }
